...
int parse_asx(QString file_content, vector<MetaData>& v_md){
	...
	doc.setContent(file_content);

	qDebug() << "set content " << file_content;
...
int parse_pls(QString file_content, vector<MetaData>& v_md){
	...
	MetaData md;
	for(int i=0; i<n_titles; i++) v_md.push_back(md);
...
int PlaylistParser::parse_playlist(QString playlist_file, vector<MetaData>& v_md){
	...
	if(playlist_file.startsWith("http")) return 0;

	QString content;
	bool success = Helper::read_file_into_str(playlist_file, content);
	if(!success) return 0;

	if(playlist_file.toLower().endsWith("m3u")){
		return parse_m3u(content, v_md_tmp);
	}

	else if(playlist_file.toLower().endsWith("pls")){
		return parse_pls(content, v_md_tmp);
	}

	else if(playlist_file.toLower().endsWith("asx")){
		return parse_asx(content, v_md_tmp);
	}
