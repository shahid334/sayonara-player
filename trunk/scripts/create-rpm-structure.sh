...
DATETIME=`date +%y%m%d`
VERSION=$1
REVISION=`head -1 revision | cut -d '#' -f 2`
REVISION_STR="1.${REVISION}"
...
CHANGELOG_ENTRY="\%changelog\n\* ${CHANGELOG_DATE} Lucio Carreras <luciocarreras@gmail.com> - ${VERSION}-${REVISION_STR}\n- write some changelog here\n"
...
sed -i "s/<<REVISION>>/${REVISION_STR}/g" ${SPEC_PATH}/sayonara.spec
...
mv sayonara-player.tar.gz ${SOURCE_PATH}/sayonara-player-${VERSION}-${REVISION_STR}.tar.gz
