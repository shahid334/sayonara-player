...
GSTPipelineExperimental::GSTPipelineExperimental(QObject* parent)
{
            ...
            do {
                ...
                _audio_src = gst_element_factory_make("giosrc", "src");
                ...
                if(!_test_and_error(_decoder, "Engine: Decoder creation fail")) break;
                ...
                gst_bin_add_many(GST_BIN(tmp_pipeline), _audio_src, _decoder, _audio_convert, _tee,
                ...
                success = gst_element_link(_audio_src, _decoder);
                _test_and_error_bool(success, "Engine: Cannot link src with dec");
                ...
                s = gst_pad_link (_tee_spectrum_pad, _spectrum_pad);
                    _test_and_error_bool((s == GST_PAD_LINK_OK), "Engine: Cannot link tee with spectrum");
                     ...
                     g_signal_connect (_decoder, "pad-added", G_CALLBACK (pad_added_handler), _tee);


                g_object_set (_audio_src, "blocksize", -1, NULL);
        ...
        /*****/
               ...
               g_object_set(G_OBJECT(_audio_src), "blocksize", 1024, NULL);
...
bool GSTPipelineExperimental::set_uri(gchar* uri){
    if(!uri) return false;
    g_object_set(G_OBJECT(_audio_src), "location", uri, NULL);
    gst_base_src_set_blocksize(GST_BASE_SRC(_audio_src), -1);
