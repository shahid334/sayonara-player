...
/*
Do a Fourier transform of an array of N complex_ff numbers separated by steps of (complex_ff) size skip.
...
void fftc1(float f[], int N, int skip, int forward)
{
  ...
  struct complex_ff wb; /* wk = W^k = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
  struct complex_ff temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complex_ff *c = (struct complex_ff *)f; /* Treat f as an array of N complex_ff numbers */
...
/*
Do a Fourier transform of an ndims dimensional array of complex_ff numbers
Array dimensions are given by size[0],...,size[ndims-1]. Note that these are sizes of complex_ff arrays.
...
void fftcn(float f[], int ndims, int size[], int forward)
{
  ...
  for(dim=ndims-1;dim>=0;dim--) /* Loop over dimensions */
  {
	...
	fftc1(f+2*(i+j),size[dim],skip,forward); /* 1-D Fourier transform. (Factor of two converts complex_ff index to float index.) */
...
void fftr1(float f[], int N, int forward)
{
  ...
  struct complex_ff wb; /* wb = W^b = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
  struct complex_ff temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complex_ff *c = (struct complex_ff *)f; /* Treat f as an array of N/2 complex_ff numbers */

  if(forward==1)
    fftc1(f,N/2,1,1); /* Do a transform of f as if it were N/2 complex_ff points */
...
The (complex_ff) nyquist frequency components are stored in fnyquist[size[0]][size[1]]...[2*size[ndims-2]]
Forward determines whether to do a forward transform (1) or an inverse one(-1)
*/
void fftrn(float f[], float fnyquist[], int ndims, int size[], int forward)
{
  ...
  struct complex_ff wb; /* wb = W^b = e^(2 pi i b/N) in the Danielson-Lanczos formula for a transform of length N */
  struct complex_ff temp1,temp2; /* Buffers for implementing recursive formulas */
  struct complex_ff *c = (struct complex_ff *)f, *cnyquist = (struct complex_ff *)fnyquist; /* Treat f and fnyquist as arrays of complex_ff numbers */
  int totalsize=1; /* Total number of complex_ff points in array */
  ...
  size[ndims-1] /= 2; /* Set size[] to be the sizes of f viewed as a complex_ff array */
  for(i=0;i<ndims;i++)
  {
    totalsize *= size[i];
    indices[i] = 0;
  ...
  {
    fftcn(f,ndims,size,1); /* Do a transform of f as if it were N/2 complex_ff points */
